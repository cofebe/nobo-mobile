import { IonItem, IonRow, IonCol, IonAvatar, IonIcon, useIonActionSheet } from '@ionic/react';
...
const PostDetailItem: React.FC<FeedListItemProps> = ({
  message,
    ...
    (!isDeleted && (
      ...
      >
        <IonRow className="feed-item-item-content ion-text-wrap">
          <IonCol size="1" style={{marginLeft: "30px"}}>
            <IonAvatar
          ...
          >
            <div className="feed-list-feed-message" style={{ paddingLeft: "64px", paddingRight: "64px" }}>
              <div></div>
              <Carousel
                showArrows={false}
                dynamicHeight={false}
              ...
              >
                {message.images.length > 0 &&
                  message.images.map((mp, i: number) => (
                  ...
                  /> */}
                      <div
                        className="nobo-post-image"
                        style={{
                          backgroundImage: `url(${mp.url.replace("'", '').replace("'", '')})`,
                        }}
                      >
              ...
              {message.images.length > 0 && (
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: message.feedText }}
                ></div>
              )}
              {message.feedText && !(message.images && message.images[0]) && (
                ...
                >
                  <div
                    className="feed-item-text feed-item-text-large"
                    dangerouslySetInnerHTML={{ __html: message.feedText }}
                  ></div>
                  <div className="nobo-image-options-container">
                    <div onClick={() => likePost(message._id)}>
              ...
              <div className="feed-item-date">
                {getTimeDifferenceString(new Date(message.createdAt))}
              </div>
              {/*              { message.images.length === 0 && (
                <div className="nobo-image-options-container">
                  <div onClick={() => likePost(message._id)}>
          ...
          <IonCol size="12" className={`feed-content feed-content-footer`}></IonCol>
        </IonRow>
