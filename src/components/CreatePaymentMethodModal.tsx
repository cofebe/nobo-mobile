...
const CreatePaymentMethodModal = forwardRef<Ref, CreatePaymentMethodModalProps>(
  ({ onClose }, ref) => {
    ...
    function validate() {
      return (
        !!firstName && !!lastName && !!address1 && !!city && !!state && !!state && !!postalCode
      );
    }
    ...
    function submit(e: Event) {
      ...
      stripe.createToken(cardNumberElem, data).then(ret => {
        ...
        userService.addPaymentMethod(ret).then((success: boolean) => {
          if (!success) {
          ...
          if (isDefault && ret.token?.card?.id) {
            userService.setDefaultPaymentMethod(ret.token?.card?.id).then((success: boolean) => {
              if (!success) {
      ...
      >
        <IonHeader className="address-header">
          <IonToolbar className="address-header-toolbar">
            <IonGrid>
              <IonRow>
                <IonCol size="12">New Payment Method</IonCol>
              </IonRow>
            </IonGrid>
          </IonToolbar>
        </IonHeader>
