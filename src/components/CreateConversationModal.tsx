...
const CreateConversationModal = forwardRef<Ref, CreateConversationModalProps>(({ onCancel, onOpen }, ref) => {
  ...
  return (
    <IonModal className="create-conversation-container" ref={ref} backdropDismiss={false} swipeToClose={false}>
      ...
      <IonContent className="create-conversation-content" scrollY={true}>
        ...
        <IonList>
          {connections
            .filter(conn => `${conn.first_name} ${conn.last_name}`.toLowerCase().includes(filter) || (conn.school || '').toLowerCase().includes(filter) || (conn.primary_position || '').toLowerCase().includes(filter))
            .map(conn => (
                  ...
                  <IonCol>
                    <div className="connection-container">
                      ...
                      <div className="name-text">
                        ...
                        <div className="name">{conn.first_name} {conn.last_name}</div>
                        <div className="position-school">{conn.primary_position} {conn.school}</div>
                      </div>
                      <div className="check">
                        <IonCheckbox onClick={(e) => {
                          e.preventDefault();
                          toggle(conn.user_id);
