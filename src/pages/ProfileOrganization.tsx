import OrganizationProfileSummary from '../components/ProfileSections/OrganizationProfileSummary';
import { useState, useRef } from 'react';
import { isPlatform } from '@ionic/react';
import { useHistory } from 'react-router-dom';
import { TrainerProfile, emptyProfile } from '../data/trainer-detail';
import { Profile } from '../data/profile';
import { OrganizationProfile, OrganizationEmptyProfile } from '../data/organization';
...
import OrganizationRosterSection from '../components/ProfileSections/OrganizationRosterSection';
...
const ProfileOrganization: React.FC<ProfileTrainerProps> = (
  ProfileOrganization
) => {
  ...
  let [organizationProfile, setOrganizationProfile] = useState<OrganizationProfile>(OrganizationEmptyProfile);
  let [organizationUsers, setOrganizationUsers] = useState<Profile[]>([]);
  const [noPlayersFound, setNoPlayersFound] = useState(false);
  let [coachProfile, setCoachProfile] = useState<CoachProfile>();

  const [socialData, setSocialData] = useState<any>({});

  const [targetSection, setTargetSection] = useState('Current Roster');
  ...
  var options = {
    // message: `${organizationProfile.organization_name}`, // not supported on some apps (Facebook, Instagram)
    subject: 'Share Organization', // fi. for email
    ...
    present({
      ...
      buttons: [
        ...
        return (
          b.who === 'all' ||
          !b.who ||
          (b.who === 'me' && ProfileOrganization.myProfile) ||
          (b.who === 'notme' && !ProfileOrganization.myProfile)
        );
      }),
      onDidDismiss: ({ detail }) => {
  ...
  function reset() {
    ...
    setProfileURL('');
    setOrganizationProfile(OrganizationEmptyProfile);
    setTargetSection('Current Roster');
    setArrayOfSocials([]);
    setSocialData({});
  }
  ...
  function loadProfile() {
    ...
    // console.log('myProfile value: ', ProfileOrganization.myProfile);
  ...
  function getProfile(id: number = 0) {
    ...
    // console.log('GetOrganization: ', id);
    ...
    organizationService
      ...
      .then((data) => {
        ...
        if (data === null) {
          ...
          organizationProfile = updateProfile;
          setOrganizationProfile(updateProfile);
          setOrganizationUsers(updateProfile.users)
          setNoPlayersFound(updateProfile.users.length === 0);
          !ProfileOrganization.myProfile &&
  ...
  return (
    <IonPage className="home-page-organization-profile" style={{ backgroundColor: '#F9FBFB' }}>
      <IonContent className="organization-profile-content" scrollY={false}>
        {!isBrowser && !ProfileOrganization.myProfile && (
          <IonButtons
            style={{
              position: 'absolute',
              left: '1.5rem',
        ...
        <IonModal ref={modal} trigger="open-modal">
          ...
          <IonContent className="ion-padding">
            <QrCode value={profileURL} name={organizationProfile.organization_name}></QrCode>
          </IonContent>
        </IonModal>
        <div
          className="profile-banner-container"
          style={{ backgroundImage: `url(${organizationProfile.banner_picture})` }}
        ...
        <OrganizationProfileSummary
          ...
          myProfile={ProfileOrganization.myProfile}
        ></OrganizationProfileSummary>
          ...
          <div>
            {ProfileOrganization.myProfile && (
              <IonButton
                onClick={(e) => {
                  e.preventDefault();
                  history.push(`edit-organization/${userId}`);
                }}
                fill="clear"
                color="#1A3A35"
...
{/*            {!isBrowser && myUserId !== trainerProfile.id && (
        ...
        {targetSection === 'Current Roster' && (
          organizationUsers.map((m: Profile) => (
            <ExploreListItem key={m.user_id} profile={m} />
          ))
        )}
        {(noPlayersFound && targetSection !== 'Past Roster') && (
          <h2 style={{ textAlign: 'center', paddingTop: '1rem' }}>
            No Users in Roster
          </h2>
        )}
        {targetSection === 'Past Roster' && (
          <h2 style={{ textAlign: 'center', paddingTop: '1rem' }}>
            Coming Soon...
          </h2>
