...
const SignUpAthlete: React.FC<Props> = props => {
 ...
 async function getProfile() {
  ...
  return await userService
   ...
   .then(data => {
    ...
    let slinksResult = processSocialLinks(basicInfo?.athlete_user_profile?.social_links?.String);
 ...
 function validateSignup(request: any) {
  ...
  if (!request?.profile_picture?.base64String && !props.editMode) {
   valid = false;
   fixTheFollowing.push('• Missing Profile Picture: Please upload a profile picture.   <br/><br/>');
  }
  if (!request?.banner_picture?.base64String && !props.editMode) {
   valid = false;
   fixTheFollowing.push('• Missing Banner Picture: Please upload a banner picture.   <br/><br/>');
  }
 ...
 return (
  <IonPage id="sign-up-athlete-page" className="nobo-page sign-up-athlete-page">
   ...
   <IonHeader>
    <IonToolbar className="toolbar-no-border" style={{ paddingTop: props.editMode ? '' : '40px' }}>
     {!props.editMode && (
     ...
     <IonButtons style={{ marginTop: props.editMode ? '10px' : '40px' }} slot="start">
      ...
      {props.editMode && (
       <IonBackButton text="&nbsp;  Edit Athlete Profile" className="nobo-nav-text"></IonBackButton>
      )}
     </IonButtons>
    </IonToolbar>
   </IonHeader>
       ...
       <IonAccordion id="basic-info" value="basic-info">
        ...
        <IonList className="nobo-list-input" slot="content">
         ...
         <IonItem className="nobo-input" lines="none">
          <IonRow>
           <IonCol>
            ...
            >
             <IonIcon className="transparent-button" src="assets/images/photo-upload.svg"></IonIcon>
             {profilePicPreview !== '' && (
              <div
         ...
         <IonItem className="nobo-input" lines="none">
          ...
          <span className="nobo-optional" style={{ paddingTop: 6, paddingLeft: '2px !important' }}>
         ...
         <IonItem className="nobo-input" lines="full">
          ...
          <UrpSelect
           ...
           options={getPositions()
            .filter(p => {
             return p.sport === normalizeSportGender(location.state) || p.sport === primarySport;
            })
            .map(p => {
             return {
          ...
          <IonGrid style={{ paddingLeft: '0px' }}>
           ...
           <IonRow>
            <IonCol style={{ paddingLeft: '0px' }}>
             <IonLabel className="nobo-signup-label" position="stacked">
              Birthday{' '}
              <span className="disclaimer-text"> Used for informational pnobooses only</span>
             </IonLabel>
            </IonCol>
            <IonCol size="5" style={{ paddingLeft: '0px' }}>
          ...
          <IonInput
           ...
           onIonChange={e => {
            ...
            const autoCompleteSchools = allSchools.filter(
             singleSchool => {
              if (count < 5 && singleSchool.toLowerCase().includes(inputtedSchool!.toLowerCase())) {
               count++;
               return true;
              }
              return false;
         ...
         <IonItem className={'nobo-input' + (socialLinkTikTokValid ? '' : ' invalid')} lines="full">
          <IonLabel className="nobo-signup-label" position="stacked">
         ...
         <IonItem className={'nobo-input' + (socialLinkHudlValid ? '' : ' invalid')} lines="full">
          <IonLabel className="nobo-signup-label" position="stacked">
         ...
         <IonItem className={'nobo-input' + (socialLinkRivalsValid ? '' : ' invalid')} lines="full">
          <IonLabel className="nobo-signup-label" position="stacked">
          ...
          return (
           <>
                ...
                <UrpSelect
                 ...
                 onChange={e => {
                  if (e?.length) {
                   ...
                   const season = statsCopy.find((seas: any) => seas.season === s.season);
                   if (season) {
                    season.season = e[0];
                    setStats(statsCopy);
                   }
            ...
            {s.categories.map((c: any) => {
             let section = sportStats.sections.find((sec: any) => sec.name === c.section);
             ...
             const category = section.categories.find((cat: any) => cat.name === c.category);
             if (!category) {
               ...
               <IonItem className="stats-category">
                ...
                <IonLabel
                 ...
                 onClick={e => {
                  e.preventDefault();
                  const statsCopy = getStatsCopy();
                  const season = statsCopy.find((seas: any) => seas.name === s.name);
                  if (season) {
                 ...
                 {category.stats.map((stat: any) => {
                  return (
                   <IonRow key={stat.name}>
                    <IonCol size="9">
                     <IonLabel className="stats-entry-label">{stat.name}</IonLabel>
                    </IonCol>
                    <IonCol size="3">
                     <IonInput
        ...
        <IonList className="nobo-list-input" slot="content">
         ...
         {isSectionActive('football') && (
          <IonItem className="nobo-input" lines="none">
           ...
           <IonRow>
            <IonCol size="7">
             <div style={{ display: 'block', cursor: 'pointer' }} className="nobo-rating-input">
         ...
         {awards.map((element: any, index: any) => {
          return (
           <div key={index}>
            ...
            <IonItem className={invalidAwardIds.includes(index) ? 'invalid' : ''} lines="full">
             <IonInput
           ...
           {awards.length > 1 ? (
            <IonLabel
             ...
             onClick={e => {
              ...
              setInvalidAwardIds(invalidAwardIds.filter(id => id !== awards.length - 1));
               ...
               <IonCol style={{ paddingLeft: '0px' }}>
                ...
                >
                 ...
                 <IonInput
                  ...
                  onIonChange={e => {
                   const value = e.detail.value!;
                   let lastPreviousTeamEndYear = previousTeams[index - 1]?.endYear;
                   if (value !== null && value !== undefined && value !== '') {
                    if (!/^[0-9]{4}$/.test(value)) {
                     ...
                     const v = +value;
                     setInvalidStartYearIds(invalidStartYearIds.filter(id => id !== index));
                  ...
                  onIonChange={e => {
                   ...
                   if (value !== null && value !== undefined && value !== '') {
                    if (!/^[0-9]{4}$/.test(value)) {
                     ...
                     const v = +value;
                     setInvalidEndYearIds(invalidEndYearIds.filter(id => id !== index));
                     if (v < 1970 || v > new Date().getFullYear() + 1) {
                      setInvalidEndYearIds([...invalidEndYearIds, index]);
                ...
                <IonItem lines="full">
                 ...
                 <UrpSelect
                  ...
                  options={getPositions()
                   .filter(p => {
                    return (
                     p.sport === normalizeSportGender(location.state) || p.sport === primarySport
                    );
                   })
                   .map(p => {
                    return {
         ...
         <IonRow>
           ...
           {previousTeams.length > 1 ? (
            <IonLabel
             ...
             onClick={e => {
              ...
              setInvalidEndYearIds(invalidEndYearIds.filter(id => id !== previousTeams.length - 1));
          ...
          <IonCol size="4" offset="4">
           <IonLabel
            style={{ color: '#2F736F', fontWeight: 'bold' }}
            onClick={e => {
             setSinglePrevTeamStateDropdownField([...singlePrevTeamStateDropdownField, '']);
             setSinglePrevTeamPositionDropdownField([...singlePrevTeamPositionDropdownField, '']);
             const newPreviousTeams = [...previousTeams];
       ...
       <IonAccordion id="player-highlights" value="player-highlights">
        ...
        <IonList className="nobo-list-input" slot="content">
         {uploadVideoOptionEnabled && (
          <IonItem lines="none">
           <UrpSelect
            placeholder={highlightLinkOrUpload === 'link' ? 'Add a Link' : 'Upload Video'}
         ...
         {highlightLinkOrUpload === 'link' ? (
          <>
           ...
           >
            <p>
             Highlight link should be a Youtube or Hudl URL. Please verify your video displays
             correctly in the preview window which should appear below after you provide a link.
            </p>
          ...
          <IonInput
           ...
           onIonChange={e => {
            ...
            const autoCompleteSchools = allUniversities.filter(
             school => {
              if (count < 5 && school.name.toLowerCase().includes(lastElement!.toLowerCase())) {
               count++;
               return true;
              } else if (
               count < 5 &&
            ...
            const arrayOfAutoCompleteSchools = autoCompleteSchools.map(school => school.name);
             ...
             return (
              <div key={offer}>
               ...
               >
                ...
                >
                 <IonIcon size="small" slot="icon-only" color="#00816d" icon={closeOutline} />
                </div>
               </div>
              </div>
             );
