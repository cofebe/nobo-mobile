...
import { UpdateCoachProfileRequest } from '../data/profile';
...
const SignUpCoach: React.FC<Props> = (props) => {
  ...
  const [previousTeamsCoached, setPreviousTeamsCoached] = useState<any>([
    {
      team: '',
      startYear: null,
      endYear: null,
  ...
  const [invalidStartYearIdsCoached, setInvalidStartYearIdsCoached] = useState<
    number[]
  >([]);
  const [invalidEndYearIdsCoached, setInvalidEndYearIdsCoached] = useState<
    number[]
  >([]);
  ...
  // EDIT MODE ADDED //
  const [profile, setProfile] = useState<UpdateCoachProfileRequest>({
    basic_user_profile: {
      first_name: {
        String: '',
        Valid: false,
    ...
    return await userService
      ...
      .then((data) => {
        ...
        let basicInfo = {
          ...
          coach_user_profile: {
            school: data.coach_user_profile.school,
            social_links: data.coach_user_profile.social_links,
            city: data.basic_user_profile.city,
            coaching_experience: data.coach_user_profile.coaching_experience,
        ...
        setPosition(basicInfo?.coach_user_profile?.position?.String);
        setCountry(
        ...
        if (basicInfo?.coach_user_profile?.coaching_experience === undefined) {
          setPreviousTeamsCoached([
            {
              team: '',
              startYear: null,
              endYear: null,
          ...
          let coachingExperiences =
            basicInfo?.coach_user_profile?.coaching_experience.map(
              (e: string) => {
                return JSON.parse(e);
              }
            );
          setPreviousTeamsCoached(coachingExperiences);
        }

        if (basicInfo?.coach_user_profile?.playing_experience === undefined) {
          setPreviousTeams([
            {
              team: '',
              startYear: null,
          ...
          let playingExperience =
            basicInfo?.coach_user_profile?.playing_experience.map(
              (e: string) => {
                return JSON.parse(e);
              }
            );
  ...
  useEffect(() => {
    ...
    if (props.editMode) {
      ...
      if (
        previousTeamsCoached &&
        previousTeamsCoached[0].team === '' &&
        bio === ''
      ) {
        setProgressStep({ step: 2, section: 'Coaching Experience' });
  ...
  function validateSignup(request: any) {
    ...
    if ((request?.position || '').length === 0 && !props.editMode) {
      valid = false;
      fixTheFollowing.push(
        '• Missing Coach Position: Please enter a position.  <br/><br/>'
      );
    }
    ...
    let req = {
      ...
      playing_experience:
        ...
        JSON.stringify([
          ...
          : previousTeams,
      coaching_experience:
        JSON.stringify(previousTeamsCoached) ===
        JSON.stringify([
          {
            team: '',
            startYear: null,
      ...
      userService
        ...
        .then((data) => {
          ...
          setTimeout(() => {
            ...
            history.push('/home/my-coach-profile');
          }, 1000);
        })
        .catch((err) => {
  ...
  async function signUpCoach() {
    ...
    console.log('SignUpCoach Req: ', req);
    let validationResults = validateSignup(req);
    if (validationResults.valid) {
      console.log('Validation Valid: ', validationResults);
      let spinner = document.querySelectorAll('.nobo-spinner-container');
      ...
      authService
        .signUpCoach(req, user.user['user_id'])
        .then((res) => res.json())
        .then((data) => {
          console.log('signUpCoach: ', data);
          authService.setUserData(data);
          ...
          <IonAccordionGroup className="nobo-section-group">
            ...
            <div className="nobo-section">
              <IonAccordion
                ...
                value="coaching-experience"
              >
                <IonItem
                  lines="none"
                  slot="header"
                  className="nobo-section-head"
                  onClick={(e) => scollToSection('coaching-experience')}
                >
                  <span className="nobo-step-num">2</span>
                  <h5 className="nobo-section-title">Coaching Experience</h5>
                </IonItem>
                <IonList className="nobo-list-input" slot="content">
                  {previousTeamsCoached.map((element: any, index: any) => {
                    return (
                      <div key={index}>
                        <IonItem className="nobo-input" lines="full">
                          <IonLabel
                          ...
                          <IonInput
                            placeholder="Team"
                            value={previousTeamsCoached[index].team}
                            autocapitalize="on word"
                            autocorrect="on"
                            type="text"
                            onIonChange={(e) => {
                              const newPreviousTeamsCoached = [
                                ...previousTeamsCoached,
                              ];
                              newPreviousTeamsCoached[index] = {
                                ...newPreviousTeamsCoached[index],
                                team: e.detail.value,
                              };
                              setPreviousTeamsCoached(newPreviousTeamsCoached);
                            }}
                            // required
                          ></IonInput>
                        </IonItem>
                          ...
                          <IonGrid style={{ paddingLeft: '0px' }}>
                            <IonRow>
                              <IonCol style={{ paddingLeft: '0px' }}>
                                <IonItem
                                  className={
                                    invalidStartYearIdsCoached.includes(index)
                                      ? 'invalid'
                                      : ''
                                  }
                                  lines="full"
                                >
                                  ...
                                  <IonInput
                                    ...
                                    type="number"
                                    value={
                                      previousTeamsCoached[index].startYear
                                    }
                                    onIonChange={(e) => {
                                      const value = e.detail.value!;
                                      let lastPreviousTeamEndYearCoached =
                                        previousTeamsCoached[index - 1]
                                          ?.endYear;
                                      if (
                                        value !== null &&
                                        value !== undefined &&
                                        ...
                                        if (!/^[0-9]{4}$/.test(value)) {
                                          setInvalidStartYearIdsCoached([
                                            ...invalidStartYearIdsCoached,
                                            index,
                                          ]);
                                        } else if (
                                          value <
                                            lastPreviousTeamEndYearCoached &&
                                          lastPreviousTeamEndYearCoached !==
                                            undefined
                                        ) {
                                          setInvalidStartYearIdsCoached([
                                          ...
                                          const v = +value;
                                          setInvalidStartYearIdsCoached(
                                            invalidStartYearIdsCoached.filter(
                                              (id) => id !== index
                                            )
                                          );
                                          if (
                                            ...
                                            invalidStartYearIdsCoached.push(
                                              index
                                            );
                                          }
                                        }
                              ...
                              <IonCol>
                                <IonItem
                                  className={
                                    invalidEndYearIdsCoached.includes(index)
                                      ? 'invalid'
                                      : ''
                                  }
                                  lines="full"
                                >
                                  ...
                                  <IonInput
                                    ...
                                    placeholder="YYYY"
                                    type="number"
                                    value={previousTeamsCoached[index].endYear}
                                    onIonChange={(e) => {
                                      const value = e.detail.value!;
                                      if (
                                        value !== null &&
                                        ...
                                        if (!/^[0-9]{4}$/.test(value)) {
                                          // isValid = false;
                                          setInvalidEndYearIdsCoached([
                                            ...invalidEndYearIdsCoached,
                                            index,
                                          ]);
                                        } else {
                                          const v = +value;
                                          setInvalidEndYearIdsCoached(
                                            invalidEndYearIdsCoached.filter(
                                              (id) => id !== index
                                            )
                                          );
                                          if (
                            ...
                            <IonRow>
                              <IonCol>
                                <IonItem lines="full">
                                  ...
                                  <IonInput
                                    value={previousTeamsCoached[index].position}
                                    placeholder="Coaching Position"
                                    autocapitalize="on word"
                                    autocorrect="on"
                                    type="text"
                        ...
                        <IonItem className="nobo-input" lines="full">
                          ...
                          <IonInput
                            value={previousTeamsCoached[index].school}
                        ...
                        <IonItem className="nobo-input" lines="full">
                          ...
                          <UrpSelect
                            value={previousTeamsCoached[index].state}
                            onChange={(e) => {
                              const newPreviousTeamsCoached = [
                                ...previousTeamsCoached,
                              ];
                        ...
                        <IonItem className="nobo-input" lines="full">
                          ...
                          <IonInput
                            value={previousTeamsCoached[index].city}
                            placeholder="City"
                            autocapitalize="on word"
                            autocorrect="on"
                            type="text"
                      ...
                      {previousTeamsCoached.length > 1 ? (
                        <IonLabel
                          ...
                          onClick={(e) => {
                            ...
                            setInvalidEndYearIdsCoached(
                              invalidEndYearIdsCoached.filter(
                                (id) => id !== previousTeamsCoached.length - 1
                              )
                            );
            ...
            <div className="nobo-section">
              <IonAccordion id="playing-experience" value="playing-experience">
                ...
                >
                  ...
                  <h5 className="nobo-section-title">Playing Experience</h5>
                </IonItem>
