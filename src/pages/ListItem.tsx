     ...
     <IonRow>
      <IonCol>
       <div className="list-item-instruction">
        ...
        {tradeSteps === 3 && (
         <>
          Select potential trade items so we know what you’re looking for. These items must be
          within a 20% bandwidth of your product
         </>
        )}
       </div>
      </IonCol>
          ...
          <IonRow key={attr.id} className="margin-bottom-5">
           <IonCol>
            <Select
             ...
             placeholder={`${getAttributeValue(attr.id).name}${attr.required ? '*' : ''}`}
             options={
        ...
        <IonCol>
         <IonRow>
          <IonCol size="12" className="input-height file-input-border">
           <label className={`${fileName ? 'opacity-full' : 'opacity-low'}`} htmlFor="fileInput">
